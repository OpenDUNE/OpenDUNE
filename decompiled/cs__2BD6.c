...
l__0024:
	...
	emu_get_memory16(emu_cs, 0x00, 0xF6) = emu_di;
	emu_addw(&emu_dx, emu_di);
	emu_get_memory16(emu_cs, 0x00, 0xFA) = emu_dx;
	emu_cld();
	emu_bx = emu_si;
	emu_dx = emu_ds;
	emu_bp = emu_es;
	goto l__003E;
l__003E:
	emu_ax = emu_get_memory16(emu_cs, 0x00, 0xFA);
	emu_subw(&emu_ax, emu_di);
	if (emu_flags.zf) goto l__009D;
	emu_get_memory16(emu_cs, 0x00, 0xF8) = emu_ax;
	emu_si = emu_bx;
	emu_ds = emu_dx;
	emu_lodsb(emu_ds);
	...
	if (emu_cx > emu_get_memory16(emu_cs, 0x00, 0xF8)) {
		emu_cx = emu_get_memory16(emu_cs, 0x00, 0xF8);
	}
	goto l__0071;
l__0071:
	emu_lodsb(emu_ds);
...
l__007E:
	...
	if ((emu_al & 0x40) != 0) goto l__00A6;
	emu_cmpb(&emu_al, 0x80);
	if (emu_al == 0x80) goto l__009D;
	emu_cl = emu_al;
