 ...
 * @implements B480:0123:0026:A929
 * @implements B480:0149:0009:8AA0
 * @implements B480:0152:0017:957F
 * @implements B480:0169:0018:55D8
 * @implements B480:018E:0002:C2BA
 * @implements B480:0190:0017:D2C7
...
l__0018:
	...
	emu_ax = 0xC5;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x0029); emu_cs = 0x01F7; emu_String_printf();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3480) { overlay(0x3480, 1); }
l__0029:
	emu_pop(&emu_cx);
...
l__0064:
	...
	emu_cwd();
	emu_cl = 0x4;
	emu_push(emu_cs); emu_push(0x007A); emu_cs = 0x01F7; emu_Tools_Shld();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3480) { overlay(0x3480, 1); }
l__007A:
...
l__0086:
	...
	emu_ax = 0x15D;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x00D7); emu_cs = 0x01F7; emu_String_printf();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3480) { overlay(0x3480, 1); }
...
l__00E4:
	...
	if (emu_get_memory8(emu_ds, 0x00, 0x98E6) == 0x0) goto l__0123;
	emu_ax = 0x1;
	emu_push(emu_ax);
	emu_ax = 0xD;
	emu_dx = 0xE2B0;
...
l__0149:
	emu_addw(&emu_sp, 0xE);
	emu_orw(&emu_ax, emu_ax);
	if (emu_ax == 0) goto l__0152;
	/* Unresolved jump */ emu_ip = 0x011B; emu_last_cs = 0xB480; emu_last_ip = 0x0150; emu_last_length = 0x0009; emu_last_crc = 0x8AA0; emu_call();
l__0152:
	emu_cmpb(&emu_get_memory8(emu_ds, 0x00, 0x98E6), 0x0);
	if (emu_get_memory8(emu_ds, 0x00, 0x98E6) == 0x0) goto l__0190;
	emu_ax = 0x40;
	emu_push(emu_ax);
	emu_xorw(&emu_ax, emu_ax);
	emu_dx = 0x1;
...
l__01CC:
	...
	emu_ax = 0x181;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x01D6); emu_cs = 0x01F7; emu_String_printf();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3480) { overlay(0x3480, 1); }
l__01D6:
	emu_pop(&emu_cx);
	emu_pop(&emu_cx);
	emu_push(emu_cs); emu_push(0x01DD); emu_cs = 0x3500; overlay(0x3500, 0); f__B500_0000_0008_FE1F();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3480) { overlay(0x3480, 1); }
l__01DD:
	emu_ax = 0x1;
