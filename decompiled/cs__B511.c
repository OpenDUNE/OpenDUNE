...
void f__B511_02CE_0023_44EC()
{
	...
	emu_push(emu_ax.x);
	emu_push(emu_dx.x);
	emu_push(emu_cs); emu_push(0x02F1); emu_cs = 0x1082; emu_Building_Find();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3511) { ovl__3511(0xFF); }
...
void f__B511_0477_000B_28F6()
{
	...
	/* Unresolved jump */ emu_ip = 0x0482; emu_last_cs = 0xB511; emu_last_ip = 0x0482; emu_last_length = 0x000B; emu_last_crc = 0x28F6; emu_call();
}
...
void f__B511_0F8B_000C_C06C()
{
	...
	emu_push(emu_cs); emu_push(0x0F97); emu_cs = 0x29E8; Input_History_Clear();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3511) { ovl__3511(0xFF); }
...
{
	emu_addws(&emu_sp, 0x8);
	emu_push(emu_cs); emu_push(0x14D8); emu_cs = 0x29E8; Input_History_Clear();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3511) { ovl__3511(0xFF); }
