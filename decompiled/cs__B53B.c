...
l__005C:
	...
	emu_push(emu_cs); emu_push(0x006B); emu_cs = 0x2B6C; emu_GUI_Mouse_Hide_Safe();
...
l__006B:
	...
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x0088); emu_cs = 0x2598; emu_GUI_Screen_SetActive();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x353B) { overlay(0x353B, 1); }
l__0088:
	emu_pop(&emu_cx);
...
l__0095:
	...
	emu_push(emu_cs); emu_push(0x00A6); emu_cs = 0x2605; emu_Font_Select();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x353B) { overlay(0x353B, 1); }
l__00A6:
	emu_pop(&emu_cx);
	...
	switch ((emu_cs << 16) + emu_ip) {
		case 0x22A6127B: emu_GFX_GetSize(); break;
		default:
...
l__0243:
	...
	emu_push(emu_cs); emu_push(0x024F); emu_cs = 0x2605; emu_Font_Select();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x353B) { overlay(0x353B, 1); }
l__024F:
	emu_pop(&emu_cx);
	...
	emu_push(emu_cs); emu_push(0x0259); emu_cs = 0x2598; emu_GUI_Screen_SetActive();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x353B) { overlay(0x353B, 1); }
l__0259:
	emu_pop(&emu_cx);
...
l__026B:
	...
	emu_push(emu_cs); emu_push(0x027E); emu_cs = 0x2B6C; emu_GUI_Mouse_Show_Safe();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x353B) { overlay(0x353B, 1); }
l__027E:
	emu_ax = 0x1;
