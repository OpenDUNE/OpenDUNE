...
void f__B518_0F65_0012_7BEF()
{
	...
	emu_push(emu_ax.x);
	emu_push(emu_dx.x);
	emu_push(emu_cs); emu_push(0x0F77); emu_cs = 0x1082; emu_Building_Find_First();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3518) { ovl__3518(0xFF); }
	f__B518_0F77_0005_922E();
}
...
void f__B518_0F7C_002C_5E19()
{
	...
	emu_push(emu_ax.x);
	emu_push(emu_dx.x);
	emu_push(emu_cs); emu_push(0x0FA8); emu_cs = 0x1082; emu_Building_Find_Next();
	/* Check if this overlay should be reloaded */
	if (emu_cs == 0x3518) { ovl__3518(0xFF); }
...
void f__B518_11AF_000D_6BEF()
{
	...
	if (!(emu_flags.zf || emu_flags.sf != emu_flags.of)) { /* Unresolved jump */ emu_ip = 0x11BC; emu_last_cs = 0xB518; emu_last_ip = 0x11B7; emu_last_length = 0x000D; emu_last_crc = 0x6BEF; emu_call(); return; }
