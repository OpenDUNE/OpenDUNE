 ...
 * @implements 1DB6:00F1:000B:03A1
 ...
 * Called From: B480:0144:0026:A929
 */
...
l__003C:
	...
	emu_push(emu_cs); emu_push(0x0055); emu_cs = 0x01F7; emu_Tools_Memcopy();
l__0055:
	emu_addw(&emu_sp, 0xA);
...
l__007E:
	...
	emu_push(emu_cs); emu_push(0x0098); emu_cs = 0x28E4; emu_Tools_PrintString();
...
l__00C5:
	...
	if (emu_get_memory16(emu_ss, emu_bp,  0x12) == 0x0) goto l__00F1;
...
l__00F1:
	...
	emu_push(emu_cs); emu_push(0x00FC); emu_cs = 0x1FB5; emu_File_Exists();
l__00FC:
	emu_pop(&emu_cx);
	emu_pop(&emu_cx);
...
l__0110:
	...
	emu_ax = 0x998E;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x0122); emu_cs = 0x01F7; emu_String_strcpy();
l__0122:
	emu_addw(&emu_sp, 0x8);
	emu_push(emu_ds);
	emu_ax = 0x9882;
...
l__012F:
	...
	emu_ax = 0x9882;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x014A); emu_cs = 0x01F7; emu_String_strcat();
l__014A:
	emu_addw(&emu_sp, 0x8);
l__014D:
	emu_push(emu_get_memory16(emu_ss, emu_bp,  0x8));
	...
	emu_ax = 0x9882;
	emu_push(emu_ax);
	emu_push(emu_cs); emu_push(0x015D); emu_cs = 0x01F7; emu_String_strcat();
l__015D:
	emu_addw(&emu_sp, 0x8);
	emu_ax = 0x33F4;
	emu_es = emu_ax;
