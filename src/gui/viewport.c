...
void GUI_Widget_Viewport_Draw(bool forceRedraw, bool arg08, bool drawToMainScreen)
{
	...
	if (g_var_39E6 != 0 || forceRedraw || updateDisplay) {
		...
		while (true) {
			...
			else { /* ENHANCEMENT - deviated unit gets colour of Deviator House */
				GUI_DrawSprite(g_screenActiveID, GUI_Widget_Viewport_Draw_GetSprite(index, (u->deviated != 0) ? (u->deviated >> 5) : Unit_GetHouseID(u)), x, y, 2, s_spriteFlags | 0xE000, s_paletteHouse, g_paletteMapping2, 1);
			}
